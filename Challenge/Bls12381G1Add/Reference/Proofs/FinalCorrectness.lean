import Challenge.Bls12381G1Add.AdditionalGoals
...
/-! # End-to-end correctness of the concrete G1ADD runtime -/

namespace Challenge.Bls12381G1Add.Reference.Proofs.FinalCorrectness
...
private def sourceRunFor (input : ByteArray) (hfit : input.size < 2 ^ 64) :=
  SourceSemantics.run_matches_spec
    (Challenge.Bls12381G1Add.ProofSupport.Yul.sourceInitialState
...
theorem reference_correctWithSchedule :
  ...
  have hframe : FrameOK (assemble Compilation.referenceInstructions)
    ...
    rw [hbytes]
    exact Challenge.Bls12381G1Add.ProofSupport.Yul.initialState_frameOK
  ...
  obtain ⟨s', hsteps, hstack, _hmatchState, hhalt⟩ :=
    hcompiled (initialState referenceBytecode input gas) hframe
      (Challenge.Bls12381G1Add.ProofSupport.Yul.sourceInitialState_matches
        referenceBytecode input gas)
      Challenge.Bls12381G1Add.ProofSupport.Yul.initialState_profile rfl rfl
  ...
  · obtain ⟨hk, hyhalt, heval⟩ :=
    ...
    cases hspec : Challenge.Bls12381G1Add.spec input with
    | none =>
        ...
        subst hkEq
        unfold Challenge.Bls12381G1Add.Matches
        rw [hspec]
    ...
    | some output =>
        ...
        subst hkEq
        simp [Challenge.Bls12381G1Add.Matches, hspec, resultOf, mkCode_toList]
...
end Challenge.Bls12381G1Add.Reference.Proofs.FinalCorrectness

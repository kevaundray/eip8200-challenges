import Challenge.Bls12381G1Add.Reference.Proofs.ByteAssemblyChunks

set_option warningAsError true

/-!
# G1ADD byte-assembly reconstruction

The expensive concrete computations live in the bounded chunk modules.  This
module combines their opaque equalities using generic list identities.
-/
...
private theorem assembleBytes_flatten (chunks : List (List Instr)) :
  ...
  | cons chunk chunks ih =>
      simp only [List.flatten_cons, assembleBytes_append, List.map_cons,
        ih]
...
private theorem referenceInstructions_split_assemble :
  ...
  rw [← flatten_chunksOf 100 10 referenceInstructions,
    assembleBytes_flatten]

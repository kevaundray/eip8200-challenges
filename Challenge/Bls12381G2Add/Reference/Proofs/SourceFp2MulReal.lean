...
/-! # Real component of frozen G2ADD `fp2Mul` -/
...
def fp2MulAfterRealReads (yst : EvmState) (a b : U256) : EvmState :=
  fp2AddReadState (fp2MulAfterV1Stores yst a b)
    (BitVec.ofNat 256 1536) (BitVec.ofNat 256 1568)
    (BitVec.ofNat 256 1600) (BitVec.ofNat 256 1632)

def fp2MulReal (yst : EvmState) (a b : U256) : U256 × U256 :=
  let s := fp2MulAfterV1Stores yst a b
  fpSubValue (loadWord s.memory 1536) (loadWord s.memory 1568)
    (loadWord s.memory 1600) (loadWord s.memory 1632)

def fp2MulRealEnv (yst : EvmState) (out a b : U256) :
    VEnv Challenge.EvmProof.modexpExec.toDialect :=
  VEnv.setMany (fp2MulV1Env yst out a b) ["\x00113", "\x00114"]
    [(fp2MulReal yst a b).1, (fp2MulReal yst a b).2]

def fp2MulAfterRealHigh (yst : EvmState) (out a b : U256) : EvmState :=
  mstoreState (fp2MulAfterRealReads yst a b) out (fp2MulReal yst a b).1

def fp2MulAfterRealStores (yst : EvmState) (out a b : U256) : EvmState :=
  mstoreState (fp2MulAfterRealHigh yst out a b)
    (out + BitVec.ofNat 256 32) (fp2MulReal yst a b).2
...
private theorem fp2MulStmt6_shape : fp2MulStmt6 =
    .assign ["\x00113", "\x00114"] fp2MulRealCall := by rfl

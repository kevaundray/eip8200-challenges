...
/-! # Non-overlapping and in-place inputs for frozen G2ADD `fp2Add` -/
namespace Challenge.Bls12381G2Add.Reference.Proofs.SourceSemantics

open YulSemantics.EVM

private theorem fp2AddAfterC0Stores_c1_before_out
...
private theorem fp2AddAfterC0Stores_c1_at_out
    (yst : EvmState) (out a b : U256)
    (hout : out.toNat + 96 < 2 ^ 256) :
    (fp2At (fp2AddAfterC0Stores yst out a b) out).c1 =
      (fp2At yst out).c1 := by
...
theorem fp2AddScheduledA_eq_before_out (yst : EvmState) (out a b : U256)
    (ha : a.toNat + 128 ≤ out.toNat)
    (hout : out.toNat + 32 < 2 ^ 256) :
    fp2AddScheduledA yst out a b = fp2At yst a := by
  apply fp2Repr_ext
  · rfl
  · exact fp2AddAfterC0Stores_c1_before_out yst out a b a ha hout

theorem fp2AddScheduledB_eq_before_out (yst : EvmState) (out a b : U256)
    (hb : b.toNat + 128 ≤ out.toNat)
    (hout : out.toNat + 32 < 2 ^ 256) :
    fp2AddScheduledB yst out a b = fp2At yst b := by
  apply fp2Repr_ext
  · rfl
  · exact fp2AddAfterC0Stores_c1_before_out yst out a b b hb hout

theorem fp2AddScheduledA_eq_at_out (yst : EvmState) (out b : U256)
    (hout : out.toNat + 96 < 2 ^ 256) :
    fp2AddScheduledA yst out out b = fp2At yst out := by
  apply fp2Repr_ext
  · rfl
